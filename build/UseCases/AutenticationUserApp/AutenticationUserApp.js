...
class AutenticationUserApp {
    ...
    async execulte(auth, tokenApp) {
        try {
            // Find User App
            let user_app = await this._authRepo.FindUserApp(auth);
            // Find App
            let app = await this._authRepo.FindAppByToken(tokenApp);
            // App is existis in user
            if (user_app.Aplicacoes_Cadastradas.indexOf(app._id.toHexString()) <= -1) {
            ...
            if (app != null && user_app != null) {
                ...
                await this._redisService.SaveTokenAutenticateUserApp(token, app, dbName, user_app.Auth_Usuario);
                return {
                    Obj: {
                        Nome: user_app.Nome,
